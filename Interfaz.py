...
def _dibujar_barra_tiempo(superficie, receta, x, y, ancho, alto=8):
    ...
    pygame.draw.rect(superficie, cfg.COLOR_FONDO, fondo, border_radius=4)
    relleno = pygame.Rect(x, y, int(ancho * frac), alto)
    if frac > 0.4:
        color = cfg.COLOR_OK
    elif frac > 0.15:
        color = cfg.COLOR_FREIDORA      # amarillo/naranja de advertencia
    else:
        color = cfg.COLOR_ALERTA
...
def dibujar_panel_ordenes(superficie, ordenes, x, y, ancho, alto,
    ...
    pygame.draw.rect(superficie, cfg.COLOR_PARED, panel)
 
    titulo = fuente_titulo.render("ORDENES", True, cfg.COLOR_TEXTO)
    superficie.blit(titulo, (x + 14, y + 10))
 
    cy = y + 40
    for receta in ordenes:
        tarjeta = pygame.Rect(x + 10, cy, ancho - 20, 66)
        pygame.draw.rect(superficie, cfg.COLOR_PISO, tarjeta, border_radius=6)
 
        # Nombre de la receta
        nombre = fuente.render(receta.nombre, True, cfg.COLOR_TEXTO)
        superficie.blit(nombre, (tarjeta.x + 8, tarjeta.y + 6))
 
        # Puntos actuales (a la derecha)
        pts = fuente.render(f"{receta.puntos_actuales} pts", True, cfg.COLOR_OK)
        superficie.blit(pts, (tarjeta.right - pts.get_width() - 8, tarjeta.y + 6))
 
        # Ingredientes requeridos (resumen)
        resumen = fuente.render(receta.resumen(), True, cfg.COLOR_TEXTO_TENUE)
...
def dibujar_fin_juego(superficie, puntaje, ancho, alto,
    ...
    centrado("TIEMPO!", fuente_grande, cfg.COLOR_TEXTO, alto // 2 - 40)
    centrado(f"Puntaje final: {puntaje}", fuente, cfg.COLOR_OK, alto // 2)
    centrado(instruccion, fuente_chica, cfg.COLOR_TEXTO_TENUE, alto // 2 + 36)
